...
export default function MarketInsightsHub() {
  const blogs = Object.entries(registry as SEORegistry)
    .filter(([key]) => key.startsWith("market-insights/"))
    .map(([key, data]) => ({
      slug: key,
      ...data
    }));
    ...
    <main className="min-h-screen bg-light">
      ...
      <section className="pt-32 pb-20 bg-primary overflow-hidden relative text-center">
         ...
         <div className="container mx-auto max-w-4xl px-4 relative z-10 space-y-6">
            <span className="text-accent font-bold uppercase tracking-[0.5em] text-[10px]">Landed Estate Intelligence</span>
            <h1 className="text-4xl md:text-6xl font-heading font-bold text-white">Market Insights</h1>
            <p className="text-white/50 max-w-2xl mx-auto leading-relaxed italic">
                Advanced analysis, ROI reports, and infrastructure updates for the selective NA plot investor in Pune East.
            </p>
         </div>
      </section>
            ...
            {blogs.map((blog, i) => (
              ...
              >
                 ...
                 <div className="pt-4 border-t border-dark/5 flex items-center justify-between">
                    <span className="text-[10px] font-black uppercase text-accent tracking-tighter">Read Full Insight</span>
