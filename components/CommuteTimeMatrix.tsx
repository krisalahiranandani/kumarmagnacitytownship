...
export default function CommuteTimeMatrix() {
  ...
  return (
    <section className="py-24 bg-[#030303] border-t border-white/10 relative overflow-hidden" id="commute-matrix">
      <div className="container mx-auto px-6 max-w-7xl relative z-10">
        <div className="text-center max-w-3xl mx-auto space-y-4 mb-16">
          ...
          {/* Mode Switcher */}
          <div className="inline-flex p-1.5 rounded-full bg-white/5 border border-white/10 mt-4">
            <button
              onClick={() => setMode("car")}
              className={`flex items-center gap-2 px-5 py-2 rounded-full text-xs font-mono transition-all ${
                mode === "car" ? "bg-accent text-dark font-bold shadow-lg" : "text-gray-400 hover:text-white"
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2 rounded-full text-xs font-mono transition-all ${
                mode === "bike" ? "bg-accent text-dark font-bold shadow-lg" : "text-gray-400 hover:text-white"
              }`}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {COMMUTES.map((item, idx) => (
            <div
              key={idx}
              className="bg-white/5 border border-white/10 rounded-3xl p-6 hover:border-accent/40 transition-all flex flex-col justify-between"
            >
              <div className="space-y-3">
                <div className="flex justify-between items-center text-[10px] font-mono text-accent border-b border-white/10 pb-3">
                  <span>{item.category}</span>
                  <span className="text-gray-400 font-mono">{item.distance}</span>
                </div>
                <h3 className="text-lg font-serif font-bold text-white">{item.destination}</h3>
              </div>

              <div className="mt-6 pt-4 border-t border-white/10 flex items-center justify-between">
                <span className="text-xs text-gray-400 font-mono">Transit Time</span>
