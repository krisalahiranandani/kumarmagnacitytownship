...
export default function InstantTokenBooking() {
  ...
  return (
    <section className="py-24 bg-warm-bg border-t border-primary/10 relative overflow-hidden" id="instant-booking">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
                ...
                <button
                  ...
                  className={`w-full text-left p-6 rounded-2xl border transition-all flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 ${
                    isSelected
                      ...
                      : "border-primary/10 bg-white hover:border-primary/20 shadow-sm"
                  }`}
                >
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-primary/60 font-mono">Possession: {unit.possession}</span>
                    </div>
                    <h3 className="text-lg font-serif font-bold text-primary">{unit.title}</h3>
                    <p className="text-xs text-primary/60 font-mono">{unit.carpet}</p>
                  </div>

                  <div className="text-left sm:text-right border-t sm:border-t-0 border-primary/10 pt-3 sm:pt-0 w-full sm:w-auto">
                    <span className="text-xs text-primary/60 block font-mono">Launch Price</span>
                    <span className="text-xl font-bold text-emerald-600 font-mono">{unit.price}</span>
                  </div>
                </button>
              );
            })}
          ...
          <div className="lg:col-span-5 bg-gradient-to-b from-primary/5 to-white border border-accent/30 rounded-3xl p-8 backdrop-blur-2xl shadow-2xl space-y-6">
            <div>
              ...
              <h3 className="text-2xl font-serif font-bold text-primary">Reserve {selectedUnit.title}</h3>
              <p className="text-xs text-primary/60 mt-1">Nominal Token Amount: <span className="text-accent font-bold font-mono">{selectedUnit.token}</span></p>
            </div>

            {isSubmitted ? (
              <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-2xl p-6 text-center space-y-3">
                <CheckCircle2 size={36} className="text-emerald-400 mx-auto" />
                <h4 className="text-lg font-serif font-bold text-primary">Inventory Lock Request Confirmed</h4>
                <p className="text-xs text-primary/80">Our sales lead manager will call you at {form.phone} within 15 minutes with the official unit cost sheet.</p>
              </div>
            ) : (
