...
export default function MortgageEligibilitySuite() {
  ...
  return (
    <section className="py-24 bg-warm-bg border-t border-primary/10 relative overflow-hidden" id="home-loan-calculator">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-7 bg-white border border-primary/10 rounded-3xl p-8 backdrop-blur-xl space-y-8 shadow-sm">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {BANKS.map((bank, idx) => (
                  ...
                  >
                    ...
                    <h3 className="text-xs font-bold text-primary">{bank.name}</h3>
                    <p className="text-[11px] text-emerald-600 font-mono font-bold">{bank.rate}% p.a.</p>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center text-sm">
                <span className="text-primary/60 font-mono">Loan Amount</span>
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center text-sm">
                <span className="text-primary/60 font-mono">Loan Tenure</span>
                <span className="text-xl font-bold text-primary font-mono">{tenureYears} Years</span>
              </div>
              <input
                type="range"
                min={5}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 pt-6 border-t border-primary/10">
              <div className="bg-primary/5 p-4 rounded-2xl border border-primary/10">
                <span className="text-[10px] uppercase font-mono text-primary/60 block mb-1">Monthly EMI</span>
                <span className="text-xl font-bold text-emerald-600 font-mono">₹{emi.toLocaleString("en-IN")}</span>
              </div>
              <div className="bg-primary/5 p-4 rounded-2xl border border-primary/10">
                <span className="text-[10px] uppercase font-mono text-primary/60 block mb-1">Total Interest</span>
                <span className="text-xl font-bold text-primary font-mono">₹{Math.round(totalInterest / 100000).toFixed(2)}L</span>
              </div>
              <div className="bg-primary/5 p-4 rounded-2xl border border-primary/10 col-span-2 sm:col-span-1">
                <span className="text-[10px] uppercase font-mono text-primary/60 block mb-1">Est. Tax Rebate</span>
          ...
          <div className="lg:col-span-5 bg-gradient-to-b from-primary/5 to-white border border-accent/30 rounded-3xl p-8 backdrop-blur-2xl shadow-2xl space-y-6">
            <div>
              ...
              <h3 className="text-2xl font-serif font-bold text-primary">Get Pre-Approved Loan Sanction</h3>
              <p className="text-xs text-primary/60 mt-1">Receive official bank pre-approval letters and custom ROI schedules.</p>
            </div>

            {isSubmitted ? (
              <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-2xl p-6 text-center space-y-3">
                <Check size={32} className="text-emerald-400 mx-auto" />
                <h4 className="text-base font-bold text-primary">Loan Pre-Approval Request Received</h4>
                <p className="text-xs text-primary/80">Our bank partner executive will contact you shortly at {leadForm.phone}.</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="text-[10px] uppercase font-mono text-primary/60 block mb-1">Full Name*</label>
                  <input
                    type="text"
                    required
                    value={leadForm.name}
                    ...
                    className="w-full bg-white border border-primary/10 rounded-xl px-4 py-3 text-xs text-primary focus:outline-none focus:border-accent shadow-sm"
                  />
                </div>
                <div>
                  <label className="text-[10px] uppercase font-mono text-primary/60 block mb-1">Phone Number*</label>
                  <input
                    type="tel"
                    required
                ...
                <div>
                  <label className="text-[10px] uppercase font-mono text-primary/60 block mb-1">Email Address</label>
                  <input
                    type="email"
