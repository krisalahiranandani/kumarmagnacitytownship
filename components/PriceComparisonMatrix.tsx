...
export default function PriceComparisonMatrix() {
  ...
  return (
    <section className="py-24 bg-warm-bg border-t border-primary/10 relative overflow-hidden" id="price-comparison">
          ...
          {INVENTORY.map((item, idx) => (
            <div
              ...
              className={`rounded-3xl p-8 flex flex-col justify-between relative transition-all duration-300 ${
                item.recommended
                  ? "bg-gradient-to-b from-accent/20 to-white border-2 border-accent shadow-2xl shadow-accent/10"
                  : "bg-white border border-primary/10 hover:border-primary/20"
              }`}
            >
              {item.recommended && (
                <span className="absolute -top-3.5 left-1/2 -translate-x-1/2 text-[10px] font-bold uppercase tracking-wider bg-accent text-white px-3 py-1 rounded-full shadow-lg">
              ...
              <div className="space-y-6">
                <div>
                  <span className="text-xs font-mono text-primary/60 uppercase font-bold">{item.tower}</span>
                  <h3 className="text-2xl font-serif font-bold text-primary mt-1">{item.type}</h3>
                ...
                <div className="border-t border-b border-primary/10 py-4">
                  <span className="text-xs text-primary/60 block font-mono">Launch All-Inclusive Price</span>
                  <span className="text-3xl font-serif font-bold text-emerald-600">{item.launchPrice}</span>
                  <span className="text-[10px] text-primary/60 block mt-1 font-mono">Possession: {item.possession}</span>
                </div>
              ...
              <button
                ...
                className={`w-full py-4 rounded-xl font-bold text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-2 mt-8 shadow-xl ${
                  item.recommended
                    ...
                    : "bg-primary/5 text-primary hover:bg-dark hover:text-white border border-primary/10"
                }`}
              >
