...
export default function Township3DVisualizer() {
  ...
  return (
    <section className="py-24 bg-warm-bg relative overflow-hidden border-t border-primary/10" id="3d-masterplan">
      ...
      <div className="container mx-auto px-6 max-w-7xl relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          <div className="lg:col-span-7 bg-white border border-primary/10 rounded-3xl p-8 relative min-h-[420px] flex flex-col justify-between backdrop-blur-xl shadow-sm">
                ...
                return (
                  <motion.button
                    ...
                    className={`relative p-6 rounded-2xl border text-left transition-all duration-300 bg-gradient-to-b ${tower.color} ${
                      isSelected ? "border-accent ring-2 ring-accent/40 shadow-2xl shadow-accent/20" : "border-primary/10 opacity-75 hover:opacity-100"
                    }`}
                  >
                    <span className="absolute -top-3 right-3 text-[9px] font-bold uppercase tracking-wider bg-accent text-white px-2.5 py-0.5 rounded-full shadow-md">
                      {tower.badge}
                    </span>
                    <Building2 className={`mb-3 ${isSelected ? "text-accent" : "text-primary/40"}`} size={32} />
                    <h3 className="text-sm font-bold text-primary mb-1">{tower.name}</h3>
                    <p className="text-[11px] text-primary/60 font-mono">G+30 Floors</p>
                  </motion.button>
                );
              })}
            </div>

            <div className="mt-8 pt-4 border-t border-primary/10 flex flex-wrap justify-between items-center text-[11px] text-primary/60 gap-4">
          ...
          <div className="lg:col-span-5">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                exit={{ opacity: 0, x: -20 }}
                className="bg-white border border-accent/30 rounded-3xl p-8 backdrop-blur-2xl space-y-6 shadow-2xl relative"
              >
                <div className="flex items-center justify-between">
                  <span className="text-xs font-mono font-bold uppercase tracking-widest text-accent">
                    {selectedTower.status}
                  </span>
                  <span className="text-xs font-mono text-primary/60 bg-primary/5 px-3 py-1 rounded-full border border-primary/10">
                    Possession: {selectedTower.possession}
                  </span>
                </div>

                <div>
                  <h3 className="text-2xl font-serif font-bold text-primary mb-2">{selectedTower.name}</h3>
                  <p className="text-xs text-primary/60">G+30 High-Rise Architecture with AAC Blocks & Gypsum Finish</p>
                </div>

                <div className="space-y-3 pt-4 border-t border-primary/10">
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-primary/60">Structure</span>
                    <span className="font-bold text-primary font-mono">G + 30 High Rise</span>
                  </div>
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-primary/60">Available Typologies</span>
                    <span className="font-bold text-accent font-mono">2 & 3 BHK</span>
                  </div>
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-primary/60">Starting Price</span>
                    <span className="font-bold text-emerald-600 font-mono text-base">{selectedTower.price}</span>
                  </div>
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-primary/60">RERA Approved</span>
                    <span className="font-bold text-primary font-mono">P52100052096</span>
                  </div>
                </div>
